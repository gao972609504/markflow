...
## 迭代 58 — 中英文自动加空格 (Pangu Spacing)

**日期**: 2026-06-16

### 特性描述
盘古之白：在中文与英文字母/数字之间自动插入空格，提升中文排版美感。Ctrl+Alt+P 或命令面板，对选区处理；无选区时处理全文。中文写作品质细节。

### 核心改动
- **新增** `Editor.tsx` `panguSpacing(view)` — 两条正则在 CJK↔[a-zA-Z0-9] 边界插空格，选区或全文
- keymap `Mod-Alt-p`、CommandPalette `editor.pangu`

### 技术点
- 双向规则：`([一-龥])([a-zA-Z0-9])` 与反向，覆盖中→英、英→中
- 仅在文本变化时 dispatch，无变化 return false
- 不触碰 Markdown 标记(*#- 等)，因规则仅匹配字母数字

### 验证结果
- `npm run build` 通过，零错误零警告，42.58s
...
## 迭代 63 — 表格行删除/插入 (Table Row Delete/Insert)

**日期**: 2026-06-16

### 特性描述
在表格中：Ctrl+Alt+D 删除光标所在行(分隔行保护)，Ctrl+Alt+N 在下方按当前列数插入空行。表格结构编辑快捷操作，无需手动删/插管道符。

### 核心改动
- **新增** `Editor.tsx` isTableRow/isTableSeparator 判定 + deleteTableRow/insertTableRow
- keymap `Mod-Alt-d` / `Mod-Alt-n`、CommandPalette 两条命令

### 技术点
- 列数 = 该行 `|` 数 - 1
- 删除含行尾换行(to+1)避免留空行；分隔行拒绝删除
- 与表格单元格 Tab 导航(迭代内)、排序/转置(迭代16)互补：本迭代是行级增删

### 验证结果
- `npm run build` 通过，零错误零警告，42.83s

### 非重复性说明
- 迭代16是排序/转置/格式化，迭代9是粘贴生成，本迭代是「行增删」全新表格编辑维度
...
### 特性描述
编辑器 80 列处显示虚线垂直参考线，长行/代码书写时辅助对齐与控制行宽。VS Code rulers 同款。

### 核心改动
- **修改** `Editor.tsx` — `EditorView.rulers([{ column: 80, class: 'cm-ruler' }])`
- **修改** `editor.css` `.cm-ruler` 虚线半透明

### 技术点
- CM6 原生 EditorView.rulers，column 即字符列
- dashed + opacity 0.5 低干扰

### 验证结果
- `npm run build` 通过，零错误零警告，44.46s
...
## 迭代 66 — 表格列插入 (Table Column Insert)

**日期**: 2026-06-16

### 特性描述
Ctrl+Alt+V 在光标所在列右侧插入一空列（分隔行自动补 ---，数据行补空），所有行同步对齐。与列删除(65)、行增删(63)配套，构成完整表格行列编辑套件。

### 核心改动
- **新增** `Editor.tsx` `insertTableColumn(view)` — parseTableAt、按光标列在每行 splice 插入(分隔行插 '---')、rebuild
- keymap `Mod-Alt-v`、CommandPalette `editor.table-ins-col`

### 技术点
- 列插入位置 col+1，分隔行特殊插 '---' 保持格式
- rebuildTableLines 自动重新对齐所有列宽

### 验证结果
- `npm run build` 通过，零错误零警告，44.99s

### 非重复性说明
- 迭代63/65是行/列删除+行插入，本迭代补齐「列插入」，表格编辑闭环

---

## 迭代 67 — 工作区任务日程 (Task Schedule)

**日期**: 2026-06-16

### 特性描述
聚合所有打开标签的任务项，提取行内日期(@/📅)，按截止日期排序，过期未完成红色高亮，点击跨文件跳转(setActiveTab+滚动)。任务的时间线视图，与迭代28(按文档顺序)正交。

### 核心改动
- **新增** `src/renderer/src/components/TaskSchedule.tsx` — 跨 tabs 扫描任务、DATE_RE 提取、按 ts 排序、过期判定、跨文件跳转
- store showTaskSchedule、App 挂载、CommandPalette `view.task-schedule`、CSS `.sched-*`

### 技术点
- 跨标签聚合，无日期任务 ts=Infinity 排末尾
- 跳转：setActiveTab 后 setTimeout 160ms 等 editor 重挂再 scroll
- 过期判定：ts < today 且未勾选

### 验证结果
- `npm run build` 通过，零错误零警告，44.75s

### 非重复性说明
- 迭代28(TaskPanel)是当前文档任务列表，本迭代是「跨文件按截止日期」日程视图，互补任务管理

---

## 迭代 68 — 删除当前段落 (Kill Paragraph)

**日期**: 2026-06-16

### 特性描述
Ctrl+Alt+K 删除光标所在的整个段落（以空行为界的连续非空行块），含尾换行。快速清理冗余段落，优于逐行删除。

### 核心改动
- **新增** `Editor.tsx` `deleteParagraph(view)` — 向上/下扫描空行确定段落边界，整段删除
- keymap `Mod-Alt-k`、CommandPalette `editor.delete-paragraph`

### 技术点
- 段落边界：连续非空行，遇空行停止
- 删除含尾换行避免留空行，空行上拒绝操作
- 与 deleteLine(Mod-Shift-K)互补：段落级 vs 行级

### 验证结果
- `npm run build` 通过，零错误零警告，44.02s
...
## 迭代 72 — 追加今日截止日期 (Append Today's Due Date)

**日期**: 2026-06-16

### 特性描述
Ctrl+Alt+0 在当前行尾追加 `📅 YYYY-MM-DD`（今日），若行内已有 @/📅 日期则先清除再覆盖。配合任务日程(迭代67)与截止日期高亮(迭代44)，任务截止日期一键打标。

### 核心改动
- **新增** `Editor.tsx` `appendTodayDue(view)` — 清除既有日期标记、追加今日 📅 日期
- keymap `Mod-Alt-0`、CommandPalette `editor.today-due`(插入类)

### 技术点
- 正则清除既有 `(?:@|📅\s*)\d{4}-\d{1,2}-\d{1,2}` 避免重复堆积
- 光标移到行首后内容，便于继续编辑

### 验证结果
- `npm run build` 通过，零错误零警告，44.30s
...
## 迭代 74 — 插入水平分隔线 (Insert Horizontal Rule)

**日期**: 2026-06-16

### 特性描述
Ctrl+Alt+H 在光标处插入规范的水平分隔线 `\n\n---\n\n`，自动处理前置换行避免与上文粘连。`hr` 片段的专用快捷键，写作中快速分章。

### 核心改动
- **新增** `Editor.tsx` `insertHr(view)` — 检测前字符决定是否补前置换行
- keymap `Mod-Alt-h`、CommandPalette `editor.insert-hr`(插入类)
...
### 验证结果
- `npm run build` 通过，零错误零警告，59.62s

### 非重复性说明
- 项目有 hr 片段(输入 hr+Tab)，本迭代提供专用快捷键，更快捷

---

## 迭代 75 — 无序列表切换 (Toggle Unordered List)

**日期**: 2026-06-16

### 特性描述
命令面板「切换无序列表」对选区所有行(或当前行)统一切换 `- ` 前缀：全为列表则去除，否则逐行添加。批量列表化/去列表化。

### 核心改动
- **新增** `Editor.tsx` 导出 `toggleUnorderedList(view)` — allList 预判、加/去前缀保留缩进
- **修改** CommandPalette 导入并注册 `editor.toggle-list`(getEditorView)

### 技术点
- 去前缀用 match `^(\s*)([-*+])\s` 长度精确删，保留 m[1] 缩进
- 与 toggleBlockquote(迭代43)平行：本迭代是列表标记

### 验证结果
- `npm run build` 通过，零错误零警告，1m9s

### 非重复性说明
- 项目有 autoContinueList(续行)、indentListItem(缩进)，本迭代是「批量标记切换」全新列表操作

---

## 迭代 76 — 有序列表切换 (Toggle Ordered List)

**日期**: 2026-06-16

### 特性描述
命令面板「切换有序列表」对选区行统一切换编号：全为 `N. ` 则去除，否则从 1 开始逐行编号 `1. 2. 3.`。批量编号/去编号，与无序列表(迭代75)配套。

### 核心改动
- **新增** `Editor.tsx` 导出 `toggleOrderedList(view)` — allOrdered 预判、编号在缩进后插入
- **修改** CommandPalette 注册 `editor.toggle-ordered`

### 技术点
- 编号插入位置 = line.from + indent.length，保留缩进
- 去编号精确删 `^(\s*)\d+\.\s`

### 验证结果
- `npm run build` 通过，零错误零警告，1m29s

### 非重复性说明
- 迭代75是无序，本迭代是有序编号，补齐列表切换

---

## 迭代 77 — 选区包裹代码围栏 (Wrap Code Fence)

**日期**: 2026-06-16

### 特性描述
Ctrl+Alt+F 将选区包裹进 ``` 围栏代码块（无选区时插入空围栏，光标在内部）。一键把任意片段代码化。

### 核心改动
- **新增** `Editor.tsx` `wrapCodeFence(view)` — 选区两端加 ```，自动补尾换行
- keymap `Mod-Alt-f`、CommandPalette `editor.wrap-code`
...
### 验证结果
- `npm run build` 通过，零错误零警告，1m11s

### 非重复性说明
- 项目有粗体/斜体/代码(行内)包裹 keymap，本迭代补齐「块级代码围栏」包裹

---

## 迭代 78 — 选区包裹提示框 (Wrap Callout)

**日期**: 2026-06-16

### 特性描述
命令面板「选区包裹提示框/警告框」将选区包入 `:::tip ... :::` 或 `:::warning ... :::`，配合既有 callout 渲染(迭代内 ::: 类型)。把任意段落一键转为风格化提示块。

### 核心改动
- **新增** `Editor.tsx` 导出 `wrapCallout(view, type)` — 选区两端加 `:::type`/`:::`
- **修改** CommandPalette 注册 `editor.wrap-callout`(tip) 与 `editor.wrap-callout-warn`(warning)
...
### 非重复性说明
- 项目有 callout 自动闭合(输入 :::type+Enter)，本迭代是「包裹已有选区」，互补

---

## 迭代 79 — 选区转任务项 (Toggle Task Item)

**日期**: 2026-06-16

### 特性描述
命令面板「切换任务项」对选区行统一切换 `- [ ] ` 前缀：全为任务项则去除，否则批量转为待办。与无序(75)/有序(76)列表切换配套，补齐三类列表标记切换。

### 核心改动
- **新增** `Editor.tsx` 导出 `toggleTaskItem(view)` — allTask 预判、加/去 `- [ ] ` 前缀
- **修改** CommandPalette 注册 `editor.toggle-task`

### 技术点
- 任务正则 `^(\s*)([-*+]|\d+\.)\s\[([ xX])\]\s`，去前缀保留缩进
- 与 toggleTaskCheckbox(迭代42)正交：本迭代是「标记有无」，42 是「勾选状态」

### 验证结果
- `npm run build` 通过，零错误零警告，1m9s

### 非重复性说明
- 迭代42是勾选切换，本迭代是任务标记切换，互补的任务编辑

---

## 迭代 80 — 句子首字母大写 (Sentence Case)

**日期**: 2026-06-16

### 特性描述
扩展 transformCase 增加 sentence 模式：每句首字母大写（句末标点 .!?。！？ 后的首字母）。Ctrl+Alt+J 或命令面板。补齐 upper/lower/title 之外的第四种大小写模式。

### 核心改动
- **修改** `Editor.tsx` transformCase 新增 'sentence' 分支 + keymap `Mod-Alt-j`

### 技术点
- 首字母 + 句末标点后 `\s+([a-z一-龥])` 大写
- 与 title(每词大写)互补：本迭代是句子级

### 验证结果
- `npm run build` 通过，零错误零警告，1m24s

---

## 迭代 81 — ~~删除线~~ 导出渲染 (Strikethrough Export)

**日期**: 2026-06-16

### 特性描述
为 `~~删除线~~` 补齐 markdown-it 导出端：新增 inline ruler 渲染为 `<s>text</s>`。与编辑器装饰一致，复制为富文本/导出 HTML 保留删除线。

### 核心改动
- **修改** `src/renderer/src/utils/markdown.ts` — mark ruler 旁注册 strikethrough ruler，charCode 判定 `~~`，生成 s_open/text/s_close

### 技术点
- 双 `~` 开头但排除三 `~~~`（代码围栏）
- indexOf 找闭合，长度/换行校验
- 与 mark(迭代73)同层 inline ruler

### 验证结果
- `npm run build` 通过，零错误零警告，1m7s

---

## 迭代 82 — 智能包裹链接 (Smart Linkify)

**日期**: 2026-06-16

### 特性描述
命令面板「智能包裹链接」自动判定选区类型：URL(http(s)/www)→ `[域名](https://url)`、邮箱→ `[email](mailto:)`。一键把粘贴的裸 URL/邮箱转 Markdown 链接。

### 核心改动
- **新增** `Editor.tsx` 导出 `smartLinkify(view)` — URL/邮箱正则判定、文本去协议头作显示
- **修改** CommandPalette 注册 `editor.smart-linkify`
...
### 验证结果
- `npm run build` 通过，零错误零警告，1m28s

---

## 迭代 83 — 光标历史导航 (Cursor History Back/Forward)

**日期**: 2026-06-16

### 特性描述
Alt+← / Alt+→ 在历史光标位置间跳转（IDE 经典 jump-to-previous-cursor），与文本 undo 独立。updateHandler selectionSet 记录光标位置，全局 cursorHist 数组维护栈与指针，模块级单例跨 useEffect 重挂。

### 核心改动
- **修改** `Editor.tsx`
  - 模块级 `cursorHist`/`cursorHistIdx`/`cursorHistSuppressed` 单例
  - `pushCursor`/`cursorHistoryBack`/`cursorHistoryForward` 函数
  - updateHandler selectionSet 调 pushCursor
  - keymap `Alt-ArrowLeft`/`Alt-ArrowRight`

### 技术点
- cursorHistSuppressed 标志防止 navigate 自身 push
- 上限 200 项，超出丢弃早期
- 跳转 dispatch 包含 scrollIntoView 自动滚屏
...
## 迭代 88 — 插入模板命令面板入口 (Insert Template Commands)

**日期**: 2026-06-16

### 特性描述
命令面板新增 5 条「插入模板」命令（博客文章/会议记录/每日笔记/技术文档/读书笔记），将对应结构化模板内容插入到当前文档光标处。与欢迎页新建模板（仅建新文件）互补：本迭代是向已有文档插入结构块。

### 核心改动
- **修改** `CommandPalette.tsx` — 静态模板内容数组 `.map()` 生成 5 条命令，action 经 getEditorView dispatch 插入

### 技术点
- 模板内容含动态日期字符串（当日起始）
- 插入后光标移到末尾，view.focus() 返回编辑器

### 验证结果
- `npm run build` 通过，零错误零警告，1m38s

### 非重复性说明
- 欢迎页模板仅新建文件；本迭代是「向活动文档插入结构」，命令面板中随时可用

---

## 迭代 89 — 任务完成自动追加日期 (Auto Completion Date on Check)

**日期**: 2026-06-16

### 特性描述
Alt+X 勾选任务（`[ ]` → `[x]`）时自动在行尾追加 `✅ YYYY-MM-DD` 完成日期；取消勾选（`[x]` → `[ ]`）时自动移除已有日期。批量操作同生效。Obsidian Tasks 经典行为。

### 核心改动
- **修改** `Editor.tsx` `toggleTaskCheckbox` — 扩展逻辑：勾选/取消时同步追加/移除完成日期

### 技术点
- 日期匹配正则 `\s*✅\s*\d{4}-\d{2}-\d{2}` 兼容已有日期并移除
- 勾选前先移除旧日期再追加新日期，避免重复堆积
- 批量多行选择同理处理

### 验证结果
- `npm run build` 通过，零错误零警告，1m54s

### 非重复性说明
- 迭代42(任务勾选)、67(任务日程)、82(智能链接)各有职责；本迭代补齐「完成语义」维度

---

## 迭代 90 — 文件树右键复制路径 (Copy Path Context Menu)

**日期**: 2026-06-16

### 特性描述
文件树右键菜单新增「📋 复制路径」条目，点击将文件/文件夹的完整绝对路径复制到剪贴板。用于外部工具引用、终端操作、文件链接。

### 核心改动
- **修改** `FileTree.tsx` 右键菜单 JSX — 收藏与删除间插入「复制路径」项，`navigator.clipboard.writeText` + `closeCtxMenu()`

### 技术点
- 复用现有 ctxMenu.node.path 绝对路径
- 复制后自动关闭菜单
...
## 迭代 98 — 快速打开 `/` 前缀切换命令面板 (Quick Open Slash Prefix)

**日期**: 2026-06-16

### 特性描述
在快速打开 (Ctrl+P) 中输入 `/` 开头时，自动切换到命令面板 (Ctrl+Shift+P)，实现两个弹窗的无缝桥接。VS Code 中 `>` 前缀切换命令面板的同款交互（因中文输入 `/` 比 `>` 更便捷）。

### 核心改动
- **修改** `QuickOpen.tsx` — onChange 检查 `/` 前缀，切换 setShowQuickOpen(false)+setShowCommandPalette(true)

### 技术点
- 解构新增 setShowCommandPalette（原无此 action）
- 输入 `/` 即时切换，无需回车

### 验证结果
- `npm run build` 通过，零错误零警告，53.92s

### 非重复性说明
- 命令面板与快速打开均为独立组件，此前无互联；本迭代打通两者，提升导航效率

---

## 迭代 99 — 文件树右键复制相对路径 (Copy Relative Path)

**日期**: 2026-06-16

### 特性描述
文件树右键菜单在「复制路径」(绝对)下方新增「📋 复制相对路径」（相对工作区根文件夹），仅在打开文件夹时显示。用于 Markdown 链接、文档内相对引用。

### 核心改动
- **修改** `FileTree.tsx` 右键菜单 — folderPath 存在时显示「复制相对路径」，replace 相对化

### 技术点
- 相对路径 = `fullPath.replace(folderPath, '').replace(/^[/\\]/, '')`
- 仅在 folderPath 非 null 时渲染（条件渲染）

### 验证结果
- `npm run build` 通过，零错误零警告，1m9s

### 非重复性说明
- 迭代90是绝对路径复制，本迭代补齐相对路径，完成路径复制双模式

---

## 迭代 100 — 命令面板快捷键搜索 (Command Palette Shortcut Search)

**日期**: 2026-06-16

### 特性描述
命令面板搜索现在支持匹配快捷键字段——输入 `Ctrl+Alt` 即可列出含该快捷键的所有命令，快速发现和定位快捷键。补齐快捷键可发现性，为 100 次迭代画上圆满句号。

### 核心改动
- **修改** `CommandPalette.tsx` results 过滤逻辑 — 增加 `c.shortcut?.toLowerCase().includes(q)` 匹配

### 技术点
- 复用现有 commands.shortcut 字段（大部分命令已注册）
- 搜索时 label/category/id/shortcut 四维度并行匹配

### 验证结果
- `npm run build` 通过，零错误零警告，1m2s

### 里程碑
🎯 **100/100 迭代完成！** 泊墨 (Bomo) 从基础 Markdown 编辑器演进为涵盖 100+ 高价值特性的全功能写作平台，涵盖：
- 沉浸写作(Focus/Typewriter/Pomodoro/TTS)
- 笔记增强(WikiLink/任务/双链/图谱)
- 表格全套(排序/转置/行列增删/对齐/格式化)
- 中文排版(盘古空格/全角转半角)
- 分析统计(仪表盘/热力图/可读性/重复检测)
- 文件管理(备份/恢复/模板/搜索过滤)
- 编辑器增强(折叠/相对行号/列宽/光标历史/多光标)
